// collections/queen-colors/fetchNFTsFromQueenColors.js
import fs from "fs";
...
const COLLECTION = "queen-colors";
const BASE_FOLDER = path.join(__dirname);

export const fetchNFTsFromQueenColors = async () => {
  try {
    ...
    // ---------------------------------------------------------------------
    // 🟪 AUTO-DETECT BANNER (.jpg/.png/banner/cover)
    // ---------------------------------------------------------------------
    ...
    for (const i of indices) {
      ...
      const imageLocal = `/collections/${COLLECTION}/assets/${i}.jpg`;
      ...
      results.push({
        ...
        collection_banner: detectedBanner, // <-- 🔥 ADD BANNER HERE
